    ...
    public <T> T create(final Class<T> service) {
        ...
        new InvocationHandler() {
          ...
          @Override public @Nullable Object invoke(Object proxy, Method method,
              ...
              //13、invoke才是最终的调用，跳转查看发现竟然是个抽象方法
              return loadServiceMethod(method).invoke(args != null ? args : emptyArgs);
              //  @}
          }
        });
...
abstract class HttpServiceMethod<ResponseT, ReturnT> extends ServiceMethod<ReturnT> {
  ...
  /**************************** invoke *******************************/  
  //21、非常的惊喜，这里具体实现了invoke方法，构建了OkHttpCall对象，内部一定是封装了Okhttp
  @Override final @Nullable ReturnT invoke(Object[] args) {
    Call<ResponseT> call = new OkHttpCall<>(requestFactory, args, callFactory, responseConverter);
    //22、但是这里没有直接返回call对象，而是再经过adapt的包装，他到底有何作用呢？
    return adapt(call, args);
  }
  /***************************** invoke ******************************/ 
    
  //23、跳转到这里发现是一个抽象方法，往下看
  protected abstract @Nullable ReturnT adapt(Call<ResponseT> call, Object[] args);
  
  //24、在构建HttpServiceMethod最终返回的是CallAdapted对象，那么我们可以猜测adapt是不是就在CallAdapted具体实现了？往CallAdapted类里看
  ...
  //27、继续跟踪retrofit.callAdapter，往上看Retrofit类
